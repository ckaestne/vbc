...
class Loader {
    ...
    def adaptMethod(owner: String, m: MethodNode): VBCMethodNode = {
        ...
        def createBlock(start: Int, end: Int): Block = {
            val instrList = for (instrIdx <- start until end;
                ...
                yield adaptBytecodeInstruction(m.instructions.get(instrIdx), methodAnalyzer.label2BlockIdx.apply)
    ...
    def adaptBytecodeInstruction(inst: AbstractInsnNode, labelLookup: LabelNode => Int): Instruction = ???
